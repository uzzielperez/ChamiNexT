...
interface ScoreBreakdownProps {
  ...
  notes?: string;
  /** Behavioral/recruiter rounds relabel the same dimensions. */
  soft?: boolean;
}

const technicalLabels: { key: keyof Omit<InterviewScores, 'overall'>; label: string }[] = [
...
const softLabels: typeof technicalLabels = [
  { key: 'thinking', label: 'Judgment & reflection' },
  { key: 'decomposition', label: 'Story structure' },
  { key: 'communication', label: 'Communication' },
  { key: 'codeQuality', label: 'Specificity & evidence' },
];
...
const ScoreBreakdown: React.FC<ScoreBreakdownProps> = ({ scores, notes, soft }) => (
  <div className="card border-accent-blue/20 p-6">
    ...
    <div className="space-y-4">
      {(soft ? softLabels : technicalLabels).map(({ key, label }) => (
        <div key={key}>
